...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="w-full min-w-0 rounded-2xl border border-border bg-card p-4 sm:p-6 space-y-4 overflow-hidden">
...
function OtpModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/40 backdrop-blur-sm overflow-y-auto">
      <div className="w-full max-w-sm min-w-0 rounded-3xl border border-border bg-background p-4 sm:p-6 space-y-4 overflow-hidden">
        ...
        <input
          ...
          placeholder="000000"
          autoFocus
          className="w-full min-w-0 rounded-xl border border-border bg-muted p-3 text-center text-xl sm:text-2xl font-bold tracking-[0.25em] sm:tracking-[0.5em] focus:outline-none focus:ring-2 focus:ring-primary"
        />
    ...
    <form onSubmit={submit} className="space-y-2">
      <div className="flex min-w-0 flex-col gap-2 sm:flex-row">
        <input
          ...
          className="w-full min-w-0 flex-1 rounded-xl border border-border bg-background p-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
        />
        <button
          type="submit"
          disabled={loading || !value.trim()}
          className="w-full sm:w-auto rounded-full bg-primary px-4 py-2.5 text-sm font-bold text-primaryForeground disabled:opacity-50 whitespace-nowrap"
        >
...
function VerifyBadge({ verified, onSend }: { verified: boolean; onSend: () => void }) {
  ...
  return (
    <button
      onClick={onSend}
      className="inline-flex shrink-0 items-center justify-center gap-1 rounded-full bg-amber-100 dark:bg-amber-900/30 px-2.5 py-0.5 text-xs font-medium text-amber-700 dark:text-amber-400 hover:bg-amber-200 transition-colors"
    >
      ⚠ Verify
    </button>
  );
...
export default function AccountPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-lg min-w-0 px-3 sm:px-4 py-8 sm:py-10 space-y-5 overflow-x-hidden">
          ...
          <div>
            ...
            {hasRealEmail ? (
              <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center">
                <p className="w-full min-w-0 flex-1 rounded-xl border border-border bg-muted px-3 py-2.5 text-sm opacity-60 truncate">
                  {user.email}
                </p>
                <VerifyBadge
