  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <p className="text-xs opacity-40">Peak: {money(max)}</p>
      </div>

      {/* Chart container — each column is a full-height flex child, the
          coloured bar grows from the bottom using margin-top:auto trick
          which is more reliable than height:% on a flex child */}
      <div className="relative h-40 flex items-end gap-[2px]">
        {series.map((d, i) => {
          const heightPct = max > 0 ? (d.revenue / max) * 100 : 0;
          const barHeight = d.revenue > 0 ? Math.max(heightPct, 4) : 0;
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-border bg-card overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-border">
            ...
            <Link href="/admin/orders" className="text-xs text-primary underline">View all</Link>
          </div>
              ...
              stats.recentOrders.map((o: any) => (
                <div key={o.id} className="flex items-center justify-between px-5 py-3">
                  <div className="min-w-0">
                    {o.customerId ? (
                      <Link href={`/admin/customers/${o.customerId}`} className="text-sm font-medium truncate hover:underline block">
                        {o.customerName}
                      </Link>
                    ) : (
