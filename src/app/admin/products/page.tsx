...
// The backend's validate middleware returns { message: 'Validation failed',
// errors: ZodError.flatten() } on a 400. Previously only the generic top-level
// message ever reached the UI ("Validation failed", with no indication of
// WHICH field or why) — making this kind of error impossible to diagnose from
// the phone alone, with no devtools access. This pulls the exact per-field
// reasons out of the response body so they render directly in the form.

function getFieldErrors(err: unknown): string[] {
  if (!(err instanceof ApiRequestError)) return [];
  const fieldErrors = (err.data as any)?.errors?.fieldErrors;
