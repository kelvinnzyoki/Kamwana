...
function AdminProductsContent() {
  ...
  //
  // FIX: previously this searched the currently-loaded paginated `products`
  // array for a matching id — which only worked if that product happened to
  // be on page 1 of results. For any product beyond the first page (e.g. a
  // low-stock item further down the catalogue), the link would silently do
  // nothing, since `.find()` never found it and no error was shown.
  //
  // Now it fetches the product directly by ID via a dedicated endpoint,
  // completely independent of pagination, search filters, or list state.
