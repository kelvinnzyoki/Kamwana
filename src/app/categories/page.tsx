...
const CATEGORY_META: Record<string, { emoji: string; description: string }> = {
  ...
  watches: {
    emoji: '⌚',
    description: 'Classic, sport, dress and designer wrist watches',
  },
  // ── Additional categories (future products) ────────────────────────────────
  ...
  accessories: { emoji: '👜', description: 'Bags, belts, scarves and jewellery' },
...
export default function CategoriesPage() {
  ...
  // Derive unique categories from live product data.
  // Falls back to the three known active categories (apparel, footwear, watches)
  // so the page is never blank on a cold load.
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-10">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm opacity-50">Browse by what you are looking for</p>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        ...
        >
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs opacity-50">Browse the full collection</p>
          </div>
        </Link>
      </div>
    </main>
