...
export const api = {
  ...
  sendPhoneOtp: (phone?: string) =>
    request<any>('/api/auth/phone/send-otp', {
      method: 'POST',
      ...(phone ? { body: JSON.stringify({ phone }) } : {}),
    }),
  verifyPhone: async (code: string, phone?: string) => {
    const data = await request<any>('/api/auth/phone/verify', {
      method: 'POST',
  ...
  sendEmailOtp: (email?: string) =>
    request<any>('/api/auth/email/send-otp', {
      method: 'POST',
      ...(email ? { body: JSON.stringify({ email }) } : {}),
    }),
  verifyEmail: async (code: string, email?: string) => {
    const data = await request<any>('/api/auth/email/verify', {
      method: 'POST',
